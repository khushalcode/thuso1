...
import {
  Lock, ArrowRight, Loader2, Eye, EyeOff,
  AlertCircle,
} from 'lucide-react'
...
export function LoginScreen({ onLoggedOut }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // Client-side auth — password-only login (no email needed).
      // auth.login will match any active user with this password.
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPass ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="pl-9 pr-10 h-11 bg-slate-800/60 border-slate-700 text-white placeholder-slate-500" required autoFocus />
            ...
            <Button type="submit" disabled={loading || !password} className="w-full h-11 bg-brand-gradient text-white hover:opacity-90 font-semibold">
              {loading ? <><Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> Signing in…</> : <>Sign In <ArrowRight className="w-4 h-4 ml-1.5" /></>}
            </Button>
          </form>
        </Card>
        <p className="text-center text-[10px] text-slate-500 mt-4">Password-only login · Multi-shop restaurant management</p>
      </motion.div>
    </div>
  )
}
