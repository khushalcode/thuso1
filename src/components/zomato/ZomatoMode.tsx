...
import {
  ArrowLeft, RefreshCw, Plus, Trash2, ChefHat, Bike, Store, Phone, MapPin, Loader2,
  CheckCircle2, Package, X, AlertCircle, Zap, Clock, Receipt,
} from 'lucide-react'
...
export default function ZomatoMode({ onExit, currentMode, onNavigate }: ZomatoModeProps) {
  ...
  // Filter by tab
  const pendingStatuses: ZomatoStatus[] = ['new', 'accepted', 'preparing', 'ready']
  const filtered = orders.filter((o) => {
    if (activeTab === 'pending') return pendingStatuses.includes(o.status)
    if (activeTab === 'delivered') return o.status === 'delivered' || o.status === 'dispatched'
    return true // all
  })
  ...
  const counts = {
    ...
    delivered: orders.filter((o) => o.status === 'delivered' || o.status === 'dispatched').length,
  }
  ...
  return (
    <div className="min-h-screen img-bg">
      ...
      <header className="sticky top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-white/20 shadow-sm">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base font-bold truncate flex items-center gap-2">
                Zomato Orders
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-rose-500 text-white">ZOMATO</span>
              </h2>
              <p className="text-[10px] text-slate-500">
                {counts.new} new · {counts.pending} pending · {counts.delivered} done
        ...
        {/* Sub-tabs: All / Pending / Delivered */}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-3 h-9">
              ...
              <TabsTrigger value="pending" className="text-xs">
                <Clock className="w-3 h-3 mr-1" /> Pending ({counts.pending})
              </TabsTrigger>
              <TabsTrigger value="delivered" className="text-xs">
                <CheckCircle2 className="w-3 h-3 mr-1" /> Done ({counts.delivered})
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 py-4">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {[0, 1, 2, 3].map((i) => <div key={i} className="h-48 bg-slate-100 rounded-2xl animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Add Manual Zomato Order</DialogTitle></DialogHeader>
...
function ManualZomatoForm({ onSubmit, onCancel }: { onSubmit: (d: any) => Promise<void>; onCancel: () => void }) {
  ...
  const [itemsRaw, setItemsRaw] = useState('Butter Chicken x1 @320\nButter Naan x2 @50')
  ...
  const submit = async () => {
    ...
    const items = lines.map((line) => {
      ...
      if (m) {
        return { name: m[1].trim(), qty: Number(m[2]), price: m[3] ? Number(m[3]) : 100 }
      }
    ...
    setSaving(true)
    try {
      await onSubmit({ customerName, customerPhone, deliveryType, address, paymentMode, items })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <Label className="text-xs">Items (one per line: "Name xQty @Price")</Label>
        <Textarea
          value={itemsRaw}
